...
PhaserGame.prototype = {
	...
	preload : function() {
		...
		this.load.tilemap('map', 'assets/maze.json', null,
	...
	update : function() {
		...
		this.marker.x = this.math.snapToFloor(Math.floor(this.car.x),
				this.gridsize)
				/ this.gridsize;
		this.marker.y = this.math.snapToFloor(Math.floor(this.car.y),
				this.gridsize)
				/ this.gridsize;
	...
	render : function() {
		...
		for (var t = 1; t < 5; t++) {
			...
			this.game.debug.geom(new Phaser.Rectangle(
					this.directions[t].worldX, this.directions[t].worldY, 32,
					32), color, true);
		}
