...
PhaserGame.prototype = {
	...
	preload : function() {
		...
		// this.load.baseURL =
		// 'http://files.phaser.io.s3.amazonaws.com/codingtips/issue002/';
	...
	create : function() {
		...
		this.targets = this.add.group(this.game.world, 'targets', false, true,
		...
		this.turret = this.add.sprite(this.tank.x + 30, this.tank.y + 14,
				'turret');

		// When we shoot this little flame sprite will appear briefly at the end
		// of the turret
	...
	bulletVsLand : function() {
		...
		if (rgba.a > 0) {
			...
			// this.land.blendDestinationOut().circle(x, y, 16, 'rgba(0, 0, 0,
			// 255').blendReset().update();
	...
	fire : function() {
		...
		// Our launch trajectory is based on the angle of the turret and the
		// power
		this.physics.arcade.velocityFromRotation(this.turret.rotation,
				this.power, this.bullet.body.velocity);
	 ...
	 * @method hitTarget
	 * @param {Phaser.Sprite}
	 *            bullet - A reference to the bullet (same as this.bullet)
	 * @param {Phaser.Sprite}
	 *            target - The target the bullet hit
	 */
	...
	/**
	 * Removes the bullet, stops the camera following and tweens the camera back
	 * to the tank. Have put this into its own method as it's called from
	 * several places.
	 * 
	 * @method removeBullet
	 */
	...
	update : function() {
		...
		if (this.bullet.exists) {
			...
			// Allow them to set the angle, between -90 (straight up) and 0
			// (facing to the right)
