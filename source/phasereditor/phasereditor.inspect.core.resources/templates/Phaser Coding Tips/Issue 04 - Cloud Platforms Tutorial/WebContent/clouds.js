...
PhaserGame.prototype = {
	...
	preload : function() {
		...
		// this.load.baseURL =
		// 'http://files.phaser.io.s3.amazonaws.com/codingtips/issue004/';
	...
	create : function() {
		...
		var cloud2 = new CloudPlatform(this.game, 800, 96, 'cloud-platform',
				this.clouds);
		...
		var cloud3 = new CloudPlatform(this.game, 1300, 290, 'cloud-platform',
				this.clouds);
	...
	checkLock : function() {
		...
		// If the player has walked off either side of the platform then they're
		// no longer locked to it
		if (this.player.body.right < this.lockedTo.body.x
				|| this.player.body.x > this.lockedTo.body.right) {
			this.cancelLock();
		}
	...
	preRender : function() {
		...
		if (this.willJump) {
			...
			if (this.lockedTo && this.lockedTo.deltaY < 0 && this.wasLocked) {
				// If the platform is moving up we add its velocity to the
				// players jump
				this.player.body.velocity.y = -300
						+ (this.lockedTo.deltaY * 10);
			} else {
				this.player.body.velocity.y = -300;
			}
	...
	update : function() {
		...
		this.physics.arcade.collide(this.player, this.clouds, this.customSep,
				null, this);

		// Do this AFTER the collide check, or we won't have blocked/touching
		// set
		var standing = this.player.body.blocked.down
				|| this.player.body.touching.down || this.locked;
...
CloudPlatform.prototype.addMotionPath = function(motionPath) {
	...
	// [
	// { x: "+200", xSpeed: 2000, xEase: "Linear", y: "-200", ySpeed: 2000,
	// yEase: "Sine.easeIn" }
	// ]
